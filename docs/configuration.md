...
| `app_version` | — | Client version stamped on Proton API requests (`x-pm-appversion`), shared by all instances. Editable in the LuCI page under **Advanced settings → Proton client version**, where **Fetch available versions** lists what the official Linux client has released; the list is only offered — you pick an entry and save it like any other setting, and nothing is ever changed for you. Empty = the version built into the package. Only set this when Proton starts rejecting the built-in version — that is **Code 5003** "this version of the app is no longer supported", not Code 2028 (a temporary account limit) and not Code 8002 (wrong credentials or unknown user) — and no package update is installed yet. Use the current official Linux client string, e.g. `linux-vpn-gtk@4.18.2` (see `versions.yml` in ProtonVPN/proton-vpn-gtk-app). Malformed values are ignored. Takes effect immediately. Read from the `globals` section when one exists, which then takes precedence over `main` — set the option there in that case |
